...
void MapDrawer::DrawCurrentRef(pangolin::OpenGlMatrix &Twc)
{
...
void MapDrawer::SetCurrentRefPose(const cv::Mat &Tcw)
{
    unique_lock<mutex> lock(mMutexRef);
    mRefPose = Tcw.clone();
}

void MapDrawer::GetCurrentOpenGLRefMatrix(pangolin::OpenGlMatrix &M)
{
    if(!mRefPose.empty())
    {
        cv::Mat Rwc(3,3,CV_32F);
        ...
        {
            unique_lock<mutex> lock(mMutexRef);
            Rwc = mRefPose.rowRange(0,3).colRange(0,3).t();
            //cout << "CameraPose\n";
	    //cout << Converter::toEulerAngles(Converter::toEigen(Rwc)) << endl;
            twc = -Rwc*mRefPose.rowRange(0,3).col(3);
	    //cout << twc.at<float>(0) << " " <<  twc.at<float>(1) << " " << twc.at<float>(2) << endl;
        }
